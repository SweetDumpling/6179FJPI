...
注: [此教程的分支版本](https://github.com/SweetDumpling/6179FJPI/tree/edit_sd?files=1)是一个更加直接的教程，处于持续维护当中，欢迎查看。
...
如果发现本教程中有错误或不足的地方，或者学到某个地方感觉无法理解而被卡住，请务必在学完后对相应位置进行修改。可以微信或者邮件找SweetDumpling加collaborator或者直接让他帮忙改~直接PR也行。

推荐在sd_edit分支上，对分支版本进行修改。因为未来分支版本可能会合并进来，此版本修改的内容如果不注意可能会丢失~

还有

### 一些约定

修改所提交的描述是赛季的年份加上第几次编辑，后面可以加括号，里面写修改内容的描述(可选)。
