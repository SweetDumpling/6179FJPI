...
以下是几条建议：
- 善用**搜索引擎**，善用**官网**，善用**官方manaul**，善用**例程**，善用**API**。
- 语言**基础要扎实**，面向对象的思想要明白。
...
### 程序结构
程序结构分为包的结构和代码的结构。可以先看示例代码的TankDrive（Sample）和PacGoat（Command-Based）。如果有些代码不明白，照着API来看。
...
### 学习API
了解了总体的结构后，就该学习WPILIBJ里的常用类和方法了。
...
### 尝试
啊哈，搭建起测试用的硬件吧。接线方法在官方给的文档里有说明，你们也可以向老队员或者老师学习接线的方法。把电机、全套气动、Servo接在一块电路里，写一个Commmand-Based，并用Joystick上的不同按钮分开控制。弄一些传感器，把信息打到SmartDashboard上。体会程序的魅力吧！别忘了编译前祈祷无Bug。
烧程序的方法在官方文档里有说明。
...
- 怎样实现按下A按钮电机全速，然后按下B按钮电机半速，松开B按钮电机全速，松开A按钮电机停止？
...
### 视觉
学习Grip和OpenCV，看API学习如何通过摄像头获得图像。看官网有通过目标像素坐标换算为实际位置的方法。
...
**流程：** 获得图像->处理->识别出目标位置->换算为实际位置->采取动作。
...
个人认为，自动阶段底盘控制较重要，手动阶段Joystick与上层机构的配合较重要。但如果队伍实力变强了，这些或许会发生变化。
...
要注意写代码的习惯。千万不要写混乱的代码，即使它能运行。更不要写构思巧妙但很难看懂的代码。而且除非必要，O(n)但生涩难懂的代码是不如O(n^2)但简单易懂的代码的。因为**一段很难“看”的代码是没有人愿意检查维护的，于是便更容易在赛场上挂掉**。（Java由于本身的特性，比C/C++更稳定易控，但最好养成这种习惯。）

还有变量名一定要准确，不要怕长，因为直观的变量名是你唯一可以不写注释的理由，而且要用英文，用英文，用英文，不要拼音。

不要企图靠注释来解释你混乱的代码。优秀的代码本身就是注释。即使别人通过读注释理解了你的代码，但混乱的代码仍然难以维护。
